...
const server = Bun.serve({
  ...
  fetch: async (req) => {
    ...
    if (url.pathname === '/app') return new Response(Bun.file('./app.html'), { headers: { 'content-type': 'text/html' } });

    if (!authorization) return new Response(null, {
      status: 401
    });
    ...
    if (url.pathname === "/stat") return Response.json({
      authorization,
    })
